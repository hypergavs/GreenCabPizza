...
global $wpdb;
if(wp_verify_nonce($_POST['secure'], 'securegensalesrep')){
$date_from_c = date_create($_POST['date_from']);	
$date_to_c = date_create($_POST['date_to']);
...
<table class="table table-condensed">
		...
		$get_amounts = $wpdb->get_results("Select 
			...
			$get_amounts = $wpdb->get_results("Select 
											sum(return_) as sold_total
											 From jd_tbl_sales Where branch LIKE '%".$cab_loc_id."%' and flavor='".$prods->flavor."' and trans_date BETWEEN CAST('".$date_from."' AS DATE) and CAST('".$date_to."' AS DATE)");
    ...
    <tr><td colspan="2"><strong>Total:</strong></td>
    	...
    	<td><strong><?php echo number_format($amt,2); ?></strong></td>
...
$(document).ready(function(e) {
	...
	var myBarChart = new Chart(ctxB, {
		...
		data: {
			...
			if($get_prods){
				foreach($get_prods as $prods){
					 if($cat=="sold"){
					 	$get_sum = $wpdb->get_row("Select sum(sold) as sold_sum From jd_tbl_sales_view Where branch LIKE '%".$cab_loc_id."%' and flavor='".$prods->flavor."' and trans_date BETWEEN CAST('".$date_from."' AS DATE) and CAST('".$date_to."' AS DATE)");
					 }elseif($cat=="unsold"){
						 $get_sum = $wpdb->get_row("Select sum(delivered) - sum(sold) as sold_sum From jd_tbl_sales_view Where branch LIKE '%".$cab_loc_id."%' and flavor='".$prods->flavor."' and trans_date BETWEEN CAST('".$date_from."' AS DATE) and CAST('".$date_to."' AS DATE)");
					 }else{
						 $get_sum = $wpdb->get_row("Select sum(return_) as sold_sum From jd_tbl_sales Where branch LIKE '%".$cab_loc_id."%' and flavor='".$prods->flavor."' and trans_date BETWEEN CAST('".$date_from."' AS DATE) and CAST('".$date_to."' AS DATE)");
					 }
	...
	$("#print-report").click(function(e){
		e.preventDefault();
		var url = $(this).attr("url") + "?secure=" + $(this).attr("secure") + "&cat=" + $(this).attr("cat") + "&d_from=" + $(this).attr("d_from") + "&d_to=" + $(this).attr("d_to");
...
<input type="button" id="print-report" class="btn btn-primary pull-right" url="<?php echo site_url() ?>/print-report/" secure="<?php echo wp_create_nonce("secureprintreport"); ?>" value="Print Report" cat="<?php echo $cat; ?>" d_from="<?php echo $date_from; ?>" d_to="<?php echo $date_to; ?>" />
<?php
}
?>